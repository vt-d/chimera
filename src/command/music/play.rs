use crate::{command::{Command, CommandContext}, State};
...
#[command(
    name = "play",
    desc = "Play a song from YouTube or other sources."
)]
    ...
    async fn execute<'ctx>(_state: State, cmd_ctx: CommandContext<'ctx>) -> Result<()> {
        let song_query = match &cmd_ctx {
            CommandContext::Prefix(prefix_ctx) => {
                Ok(prefix_ctx.parsed.remainder().to_string())
            }
            CommandContext::Slash(slash_ctx) => {
                slash_ctx.data.options.get(0)
        ...
        cmd_ctx.reply(format!("Now playing: {}", song_query)).await?;
        Ok(())
    }
}
